...
@ScalaJSDefined
trait StreamAdapter[CustomStream[_, _ <: js.Error], CustomMemoryStream[_, _ <: js.Error]] extends js.Object {

  /** Convert Origin stream to XStream */
  def adapt[T, E <: js.Error](
  ...
  def remember[T, E <: js.Error](stream: CustomStream[T, E]): CustomMemoryStream[T, E]

  /** What exactly does this method do... ? @TODO[Docs] */
  def makeSubject[T, E <: js.Error](): Subject[T, E]
  ...
  def isValidStream(stream: CustomStream[_, _ <: js.Error]): Boolean

  /** Subscribe a Cycle observer to a XStream */
  def streamSubscribe[T, E <: js.Error](stream: CustomStream[T, E], observer: Observer[T, E]): OptionalDisposeFunction
}
